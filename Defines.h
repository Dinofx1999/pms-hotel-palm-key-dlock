...
enum ERROR_TYPE
{
    ...
    ERR_BANDING         =   -34,      // 信息绑定失败
    ERR_OldREADER_NewCLIENT_DIS = -35,  // 禁止旧版发卡器读取新授权卡
    ERR_OldCLIENT_DIS   =   -36,    // 禁止降级用旧加密方式的授权卡
    ERR_READER_TYPE     =   -37,    // 读卡器类型错误
    ERR_CRC             =   -38,    // CRC校验错误

    ERR_RF50_BASE       =   8000,   // RF50动态库错误

};

#define ERR_RF50(x)     (ERR_RF50_BASE+ UINT32(x))
...
enum CARD_FLAGS
{
    ...
    CF_SET_MOTOR_DIR    = (0x01 << 24),   // 设置马达方向
};
...
struct CARD_INFO{
    ...
    char    cRFU2[22];              // 预留
    int     iExtraFlags;            // bit0==0: 旧加密, bit0==1: 新加密
